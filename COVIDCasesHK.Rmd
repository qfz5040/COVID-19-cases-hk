...
```{r message=FALSE}
#buildingsMap <-
#  buildings %>% group_by(longitude, latitude, buildingName, district) %>% summarise(count = n()) %>%
#  leaflet() %>%   
#  addTiles() %>%         
#  setView(114.15, 22.37, zoom = 11) %>%
#  addCircleMarkers(weight = 1, radius = ~ 3 * sqrt(count), color = "red", popup = ~ paste(count, "cases"))
#buildingsMap
```
